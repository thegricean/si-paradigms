...
data <- read_csv("experiments/pilot/3_processed_data/pilot_data.csv")
data$response <- pilot_data$response %>% tolower()
...
pragmatic_trials[pragmatic_trials$response=="right" | pragmatic_trials$response=="true",]$high <-0

# low definition only considers the lowers point on the scale as implicature 
pragmatic_trials$low<-0
pragmatic_trials[pragmatic_trials$response=="wrong" | pragmatic_trials$response=="false",]$low <-1

implicature_rate <- 
  pragmatic_trials %>% gather("definition","implicature", high:low)

# changing trial type names to adhoc vs. scalar
...
100 participants (20 per condition) were recruited using Amazon Mechanical Turk. 
...
```{r}
implicature_analysis_a <- glmer(implicature ~ definition * response_type * trial_type + (1+response_type|trial_type) + (1|participant), family="binomial", data=implicature_rate)
