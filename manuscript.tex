...
scalar implicature rate}
  ...
  \authornote{
    ...
    Jasbi, Department of Linguistics, Boylston Hall, Cambridge, MA,
    94305-2150. E-mail:
    \href{mailto:masoud_jasbi@fas.harvard.edu}{\nolinkurl{masoud_jasbi@fas.harvard.edu}}
  }
...
In addressing all of these questions, it has been important to obtain
...
Degen and Goodman (2014); Katsos and Bishop (2011); Sikos et al (this issue)). Indeed, the scalar
...
children to distribute a small, a big, or a huge strawberry to a puppet depending on
\enquote{how good the puppet said it}. The result was that children gave
on average smaller strawberries to the puppet when he produced
...
assumes two things: first, that whatever participants are doing in a
...
\enquote{right.}\footnote{An open question concerns the extent to which
  ...
  vs. \enquote{disagree}--\enquote{agree}). Studies vary in the labeling of scale points.} In the binary case, this reproduces the standard
...
implicature rates by condition and link. It is immediately apparent that there is variability in inferred implicature rate. In particular, the Weak link appears to result in greater estimates of
...
implicature rates with a weak link than with a strong link, but there was no evidence of a link-dependent difference in inferred implicature rate for the binary and ternary task. Finally, there was a three-way interaction between
link, trial type, and number of response options, driven by the binary/quinary contrast (Mean Estimate = -7.74,
...
In sum, both number of response options and link
affected the inferred implicature rate, as did the type of inference (exhaustive vs.~scalar).
...
speaker (\(S_1\) in the language of RSA) rather than of a pragmatic
listener who interprets utterances (\(P_{L_1}\)).\footnote{Degen and Goodman (2014) argue that sentence verification is more plausibly construed as a production task rather than as an interpretation task because participants, unlike in natural language comprehension, are provided with the ground truth about the state of the world that a speaker is describing. Thus, participants are in essence being asked to assess the quality of a speaker's utterance. In contrast, Degen and Goodman argue, true interpretation tasks are characterized by the listener inferring what the state of the world is that the speaker is describing, for instance by selecting from one of multiple interpretation options.} In their paper, Degen
\& Goodman show that sentence verification tasks are relatively more
sensitive to contextual features like the
...
\& Jaeger, 2015; Yildirim, Degen, Tanenhaus, \& Jaeger, 2016).

However, if we should like to keep the above simplifying assumptions in place, then
...
threshold value is sampled from this distribution. By allowing values of \(\theta\) to vary stochastically in this way, we can capture that \(S_1\)(u \textbar{} s) can fall both above and below \(\theta\) for a given
participant.
...
model. We leave this systematic, quantitative investigation to future work. For now we
emphasize that explicit computational modeling of behavioral responses is a tool that is  available to researchers in experimental
