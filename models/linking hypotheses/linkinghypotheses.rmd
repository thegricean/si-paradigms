---
title: "Linking Hypotheses and SI-Paradigms"
...
In order to explore this hypothesis, we first need to model S1' (specified below as speaker_filtered). Speaker_filtered is a function of utterancePrior_filtered - an utterance prior function which takes as its input only high-probability utterances; that is, utterances for which S1(u|w) > $\theta$ is true.  
...
With this model, we could revisit the logistic regression linking function explored above: the log odds of categorical response is a function of the (filtered) speaker probability of that utterance in a particular world state w and given a probability threshold $\theta$.  